...
class DepreciacaoController:
    ...
    def _connect_signals(self) -> None:
        if self._btn_filtrar:
            self._btn_filtrar.clicked.connect(self.apply_filters)
        if self._btn_limpar:
            self._btn_limpar.clicked.connect(self.clear_filters)
    ...
    def _populate_categories(self) -> None:
        ...
        current_value = self._category_combo.currentData() if self._category_combo.count() else None
        try:
        ...
        self._category_combo.addItem("Todas as categorias", None)
        ...
        if current_value is not None:
            index = self._category_combo.findData(current_value)
            if index >= 0:
    ...
    def _buscar_depreciacoes(self) -> List[Dict[str, object]]:
        ...
        texto_busca = self._search_input.text().strip() if self._search_input else ""
        if texto_busca:
            filtros.append("p.nome LIKE %s")
            parametros.append(f"%{texto_busca}%")

        categoria_id = None
        if self._category_combo and self._category_combo.currentIndex() > 0:
